...
class SimplexNoise {
    public:
        ...
        void randomizeSeed();
        double signedFBM(const double &xPos, const double &yPos, const NoiseData &noise_data); //, const unsigned int &octaves, const double &lacunarity, const double &gain);
        double signedRawNoise(const double &xPos, const double &yPos);
        double unsignedFBM(const double &xPos, const double &yPos, const NoiseData &noise_data); //, const unsigned int &octaves, const double &lacunarity, const double &gain);
        double unsignedRawNoise(const double &xPos, const double &yPos);
        void setSeed(const unsigned int _seedNumber);
