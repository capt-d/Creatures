...
void SimplexNoise::randomizeSeed() {
    long unsigned int seed = static_cast<long unsigned int>(time(nullptr));
    std::default_random_engine randomDevice(seed);
    setSeed(randomDevice());
}

void SimplexNoise::setSeed(const unsigned int _seedNumber) {
        seedNumber = _seedNumber;
		std::mt19937 seed(_seedNumber);
        std::shuffle(std::begin(permutation), std::end(permutation), seed);
}

unsigned int SimplexNoise::getSeed(void)
{
	return seedNumber;
}
 ...
 */
double SimplexNoise::signedFBM(const double &xPos, const double &yPos, const NoiseData &noise_data) {
    double sum = 0;
    double frequency = 1.0;
    double amplitude = 1.0;
    double maxValue = 0;  // Used for normalizing result between -1.0 and 1.0
    for (unsigned int i = 0; i < noise_data.octaves; ++i) {
        sum += signedRawNoise(xPos * frequency, yPos * frequency) * amplitude;
        maxValue += amplitude;
        amplitude *= noise_data.gain;
        frequency *= noise_data.lacunarity;
    }

    return sum / maxValue;
}

double SimplexNoise::unsignedFBM(const double &xPos, const double &yPos, const NoiseData &noise_data) {
    return signedFBM(xPos, yPos, noise_data) / 2.0 + 0.5;
}
