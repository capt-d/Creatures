...
void Ui_Picture::setupUi(QWidget *picture)
{
        ...
        //picture->setMinimumSize(QSize(380, 230));
        picture->setWindowTitle(QStringLiteral(""));
        verticalLayoutWidget = new QWidget(picture);
 ...
 void Ui_Picture::retranslateUi(QWidget *picture)
    {
        scale_label->setText(QApplication::translate("Picture", "Skala", Q_NULLPTR));
        pause_label->setText(QApplication::translate("Picture", "Przerwa mi\304\231dzy cyklami x100 [ms]", Q_NULLPTR));
        pause_button->setText(QApplication::translate("Picture", "Start", Q_NULLPTR));
        end_button->setText(QApplication::translate("Picture", "Koniec", Q_NULLPTR));
        Q_UNUSED(picture);
    } // retranslateUi


Ui::Picture::Picture(std::shared_ptr<const unsigned char[]> _data, unsigned int _cols, unsigned int _rows):Ui_Picture()
{
  setupUi(this);
  image_data = _data;
  timer = new QTimer();
  if (image_data)
	image = new QImage(image_data.get(), _cols, _rows, _cols, QImage::Format_Grayscale8);
  ...
  scale_value->setRange(1,10);
  pause_factor->setValue(5);
  pause_factor->setRange(1,50) ;
  
  connect(timer, &QTimer::timeout, this, &Ui::Picture::refresh);
  ...
  connect(pause_button, &QPushButton::clicked, this, &Ui::Picture::run_stop);
  connect(end_button, &QPushButton::clicked, this, &Ui::Picture::close);
  run = 0;
  window_title.clear();
...
void Ui::Picture::pause_changed(int _value) 
{
  timer->setInterval(_value*100);
}

void Ui::Picture::ustaw_wielkosc(void)
{ 
